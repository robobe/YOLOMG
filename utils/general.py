...
def check_requirements(requirements=ROOT / 'requirements.txt', exclude=(), install=True):
    ...
    if isinstance(requirements, (str, Path)):  # requirements.txt file
        ...
        with file.open() as f:
            requirements = [f'{x.name}{x.specifier}' for x in map(Requirement, (line for line in f if line.strip() and not line.startswith('#'))) if x.name not in exclude]
    ...
    for r in requirements:
        try:
            ...
            installed = parse_version(metadata.version(requirement.name))
            assert not requirement.specifier or installed in requirement.specifier
